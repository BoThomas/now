...
For core changes, also run `./scripts/test-core.sh` and
`NOW_TEST_CONFIGURATION=release ./scripts/test-core.sh`. These use the host Swift toolchain without
the macOS SDK wrapper. On a Linux devbox, record the macOS build/GUI/signing gates as unavailable;
core tests do not replace them. Analysis-tooling changes can additionally exercise the portable
compiler gates with `python3 scripts/analysis-smoke.py --compiler-only`.
