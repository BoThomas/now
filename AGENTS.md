...
- **Auto-updater** (full plan: `docs/plans/auto-updater.md`): updates itself from GitHub Releases — no Sparkle, no appcast, nothing hosted beyond GitHub.
  ...
  - **The app never swaps itself**: `UpdateInstaller` spawns a detached `/bin/sh -c` helper (params via ENV, never interpolated — remote data must not reach a shell). Helper: bounded `kill -0` poll (60 s, env-overridable) → old→backup sibling → staged→app → `open` → backup→Trash. Every failure restores first, then relaunches the OLD app with `--env NOW_UPDATE_ERROR=<reason>` (app shows the error window and never auto-offers that version again); last ditch opens the releases page. `mv` cannot replace an existing app dir — hence the `rm -rf` before restore in `fail()`.
  - **Ordering/invariants**: decide-before-download (`decide()` on the manifest first; age gate 24 h for automatic checks, 0 for manual — deleting a bad GitHub release is the rollback brake: it stops the spread but is not an undo); staging dir is a dot-prefixed SIBLING of the bundle (same volume → install is a rename; LaunchServices never indexes hidden dirs; cleaned on launch along with `now.app.old-*`). Throttle: success 24 h / failure ≥ 1 h, ≤ 3/day. Escalation: auto-checks only light the menu item + About badge; the window auto-shows once per version after 3 days uninstalled (`firstSeenUpdateDate`); manual checks always answer with the window (a menu click dismisses the menu — a transient title is never feedback). `lastNotifiedVersion` is set when the window is SHOWN, never when a check lands.
  ...
  - **Testing**: `--update-check` prints what the updater sees (read-only). `scripts/update-smoke.sh` fakes everything locally (python http.server + a PlistBuddy-bumped, re-signed "9.9.9" — the re-sign invocation must MIRROR build-app.sh's codesign line; keep the sync comments). The relaunched child reports via `open --env NOW_SMOKE_REPORT` and exits before the UI. Needs the stable identity (ad-hoc can't pass the DR gate — by design). The staging dir is HIDDEN (`.now-update-*` — `ls` needs `-a` when debugging). 404 from `/releases/latest` (no releases) = silent up-to-date; GitHub 403s without a User-Agent, so the updater pins `now/<version>`. `--update-smoke` exit codes: 0 spawned helper / 2 REFUSED / 3 up-to-date / 4 error; `NOW_UPDATE_API_BASE`/`NOW_UPDATE_REPO`/`NOW_UPDATE_TOKEN` (token only for overridden bases) are test hooks.
