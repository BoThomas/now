...
enum CalendarURL {
    ...
    @MainActor static func confirmInsecureHTTP(_ urlString: String) -> Bool {
        ...
        GeometryReader { viewport in
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    if !persistence.issues.isEmpty {
                        VStack(alignment: .leading, spacing: 10) {
                            Label("Saved data needs attention", systemImage: "exclamationmark.triangle").font(.headline)
                            ForEach(persistence.issues.keys.sorted(), id: \.self) { key in
                                HStack(alignment: .top) {
                                    Text(persistence.issues[key] ?? "").textSelection(.enabled)
                                    Spacer()
                                    Button("Reviewed") { persistence.reviewed(key) }
                                }
                            }
                        }
                        .padding(16)
